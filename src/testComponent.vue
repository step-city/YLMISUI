    ...
    <div style="height:100%;">
        <yl-panelpage :reloadIsShow="true" titleName="测试" :icon="'icon-search'">
            <div slot="content">
                    ...
                    <div slot="fristbox">
                        <yl-toolbar>
                            <el-form :inline="true">
                                ...
                                <el-form-item class="form-content-vertical">
                                    <el-date-picker v-model="searchModel.orderDate" type="year" size="small" :editable="false" placeholder="选择年份" style="width:120px">
                                    </el-date-picker>
                                </el-form-item>
                                <el-form-item class="form-content-vertical">
                ...
                <!--打印明细界面-->
                <el-dialog ref="printDialog" title="预览打印" v-model="printDialogVisible" size="large" top="8%" :lock-scroll="true" :close-on-press-escape="false" :modal-append-to-body="false">
                    <iframe :src="reportUrl" width="100%" height="600px">
                    </iframe>
                </el-dialog>
                <!--审批界面-->
                <el-dialog ref="approveDialog" title="审批" v-model="approveFormVisible" size="small" top="15%" :lock-scroll="true" :close-on-press-escape="false" :modal-append-to-body="false" :close-on-click-modal="false">
                    <roleApprove :apiUrl="'/api/services/app/gHInitialPlanOrder/ApproveGHInitialPlanOrder'" :propsOrderId="selectRows[0].id" @close="_close" v-if="approveFormVisible">
                    </roleApprove>
                </el-dialog>
                <!--审批状态界面-->
                <el-dialog ref="stateDialog" title="审批流程状态" v-model="stateFormVisible" size="small" top="15%" :lock-scroll="true" :close-on-press-escape="false" :modal-append-to-body="false" :close-on-click-modal="false">
                    <approveState v-if="stateFormVisible">
                    </approveState>
                </el-dialog>
                <!--编辑界面-->
                <el-dialog ref="mainDialog" title="编辑" v-model="addFormVisible" size="large" top="8%" :lock-scroll="true" :close-on-press-escape="false" :modal-append-to-body="false" :close-on-click-modal="false">
                    <Edit :selectRow="currentRows" @close="_close" v-if="addFormVisible">
                    </Edit>
                </el-dialog>
            </div>
        </yl-panelpage>
...
import Edit from './pages/materials/ghinitialplan/Edit';
